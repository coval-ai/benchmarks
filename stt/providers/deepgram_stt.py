...
class DeepgramProvider(STTProvider):
    def __init__(self, api_key: str, model: str = "default"):
        ...
        if model not in valid_models:
            raise ValueError(
                f"Invalid Deepgram model: {model}. Valid models: {valid_models}"
            )
    ...
    async def _receive_responses(self, ws, result: TranscriptionResult):
            ...
            async for message in ws:
                ...
                if result.ttft_seconds is None and result.audio_start_time is not None:
                    ...
                    result.first_token_content = (
                        f"Message type: {response.get('type', 'unknown')}"
                    )
        ...
        if final_flag_segments:
            ...
            if result.partial_transcripts:
                result.complete_transcript = max(
                    result.partial_transcripts, key=len
                ).strip()
