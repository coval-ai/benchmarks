...
class AssemblyAIProvider(STTProvider):
    def __init__(self, api_key: str, model: str = "default"):
        ...
        if model not in valid_models:
            raise ValueError(
                f"Invalid AssemblyAI model: {model}. Valid models: {valid_models}"
            )
    ...
    def _build_websocket_url(self, sample_rate: int) -> str:
        ...
        if sample_rate != 16000:
            print(
                f"Warning: AssemblyAI requires 16kHz audio. Got {sample_rate}Hz - this may cause errors."
            )
    ...
    async def _receive_responses(self, ws, result: TranscriptionResult):
            ...
            async for message in ws:
                ...
                if msg_type == "Turn":
                    ...
                    if response.get("end_of_turn", False):
                        ...
                        last_final_turn_time = (
                            current_time  # Track when last final turn was received
                        )

                        # Check if this looks like a formatted result (has punctuation, proper numbers)
                        is_formatted = (
                            any(char in transcript for char in ["£", ".", "-"])
                            or transcript[0].isupper()
                        )
                        if is_formatted:
        ...
        if last_final_turn_time and result.audio_start_time:
            result.audio_to_final_seconds = (
                last_final_turn_time - result.audio_start_time
            )
