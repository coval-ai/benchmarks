...
def process_transcription_result(
    result: TranscriptionResult, ground_truth: str, audio_duration: float
) -> TranscriptionResult:

    if result.complete_transcript:
        try:
            wer_analysis = compare_transcription(
                ground_truth, result.complete_transcript
            )
            ...
            if custom_wer is not None:
                ...
                if custom_wer > 0 and len(wer_analysis["incorrect_words"]) > 0:
                    logging.info(
                        f"Custom WER errors: {wer_analysis['incorrect_words']}"
                    )
            else:
    ...
    if (
        result.audio_to_final_seconds is not None
    ...
    if result.complete_transcript:
        wer_text = (
            f" (WER: {result.wer_percentage:.1f}%)"
            if result.wer_percentage is not None
            else ""
        )
        audio_to_final_text = (
            f" (Audio→Final: {result.audio_to_final_seconds:.2f}s)"
            if result.audio_to_final_seconds is not None
            else ""
        )
        rtf_text = (
            f" (RTF: {result.rtf_value:.2f}x)" if result.rtf_value is not None else ""
        )
...
def load_common_voice_sample(
    min_duration: float = 2.0, max_duration: float = 15.0, max_retries: int = 10
) -> tuple[bytes, int, int, int, str, float, str]:
    print(f"Loading Common Voice dataset (English test split)...")

    # Load dataset in streaming mode (no download)
    try:
        dataset = load_dataset(
            "mozilla-foundation/common_voice_16_1", "en", split="test", streaming=True
        )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if min_duration <= duration <= max_duration:
                ...
                audio_data = convert_to_wav_bytes(
                    audio_array, sample_rate, target_sample_rate=16000
                )
                ...
                print(
                    f"Sample {attempt+1}: {duration:.2f}s (outside {min_duration}-{max_duration}s range)"
                )
...
def convert_to_wav_bytes(
    audio_array, original_sample_rate: int, target_sample_rate: int = 16000
) -> bytes:
    ...
    if original_sample_rate != target_sample_rate:
        ...
        audio_array = librosa.resample(
            audio_array, orig_sr=original_sample_rate, target_sr=target_sample_rate
        )
    else:
    ...
    with io.BytesIO() as wav_buffer:
        sf.write(
            wav_buffer, audio_array, target_sample_rate, format="WAV", subtype="PCM_16"
        )
        wav_buffer.seek(0)
...
def save_results_to_csv(
    results: List[TranscriptionResult], timestamp: str, audio_filename: str = "test.wav"
):
    ...
    try:
        ...
        print(
            f"Contains {len(rows)} metric rows (TTFT, WER & RTF) from {len([r for r in results if not isinstance(r, Exception)])} providers"
        )
...
def load_wav_file(file_path: str) -> tuple[bytes, int, int, int, str, float]:
    ...
    with wave.open(file_path, "rb") as wav_file:
        ...
        print(
            f"Loaded audio: {channels} channels, {sample_rate}Hz, {sample_width} bytes/sample, {len(audio_data)} bytes, {duration_seconds:.2f}s duration"
        )
        return (
            audio_data,
            channels,
...
def display_results_table(results: List[TranscriptionResult]):
    ...
    print("=" * 140)
    print(
        f"{'Provider/Model':<20} {'TTFT (s)':<8} {'WER (%)':<8} {'Total (s)':<9} {'VAD (s)':<8} {'First Token':<30} {'Error':<15}"
    )
    print("-" * 140)
    ...
    for result in results:
        ...
        first_token = (
            (result.first_token_content[:27] + "...")
            if result.first_token_content and len(result.first_token_content) > 30
            else (result.first_token_content or "")
        )
        error = (
            result.error[:12] + "..."
            if result.error and len(result.error) > 15
            else (result.error or "")
        )

        print(
            f"{provider_name:<20} {ttft:<8} {wer:<8} {total:<9} {vad:<8} {first_token:<30} {error:<15}"
        )
...
def display_analysis(
    successful_ttft: List[tuple], successful_wer: List[tuple], failed_results: List[str]
):
    if successful_ttft:
        ...
        if len(successful_ttft) > 1:
            avg_ttft = sum(result[1] for result in successful_ttft) / len(
                successful_ttft
            )
    ...
    deepgram_ttft = [
        (name, time_val)
        for name, time_val in successful_ttft
        if name.startswith("deepgram")
    ]
    deepgram_wer = [
        (name, wer_val)
        for name, wer_val in successful_wer
        if name.startswith("deepgram")
    ]
    ...
    if deepgram_ttft:
        ...
        print(
            f"Deepgram TTFT fastest: {deepgram_ttft[0][0]} ({deepgram_ttft[0][1]:.3f}s)"
        )
    if deepgram_wer:
    ...
    google_wer = [
        (name, wer_val) for name, wer_val in successful_wer if name.startswith("google")
    ]
    ...
    speechmatics_ttft = [
        (name, time_val)
        for name, time_val in successful_ttft
        if name.startswith("speechmatics")
    ]
    speechmatics_wer = [
        (name, wer_val)
        for name, wer_val in successful_wer
    ...
    if speechmatics_ttft:
        ...
        print(
            f"Speechmatics TTFT fastest: {speechmatics_ttft[0][0]} ({speechmatics_ttft[0][1]:.3f}s)"
        )
    if speechmatics_wer:
        speechmatics_wer.sort(key=lambda x: x[1])
        print(
            f"Speechmatics WER best: {speechmatics_wer[0][0]} ({speechmatics_wer[0][1]:.1f}%)"
        )
...
async def main():
    ...
    print(
        f"Ground Truth Length: {len(ground_truth.split())} words, {len(ground_truth)} characters"
    )
    print(f"Audio Source: {audio_filename}")

    tasks = [
        provider.measure_ttft(
            audio_data, channels, sample_width, sample_rate, 0.1, audio_duration
        )
        for provider in providers
    ]
    ...
    for result in results:
        if isinstance(result, Exception):
            ...
            processed_results.append(
                process_transcription_result(result, ground_truth, audio_duration)
            )

    successful_ttft, successful_wer, failed_results = display_results_table(
        processed_results
    )
