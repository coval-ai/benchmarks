...
def save_results_to_csv(results: List[TranscriptionResult], timestamp: str, audio_filename: str = "test.wav"):
    """Save benchmark results to CSV file in long format with TTFT, WER and RTF metrics."""
...
async def main():
    ...
    providers = [
        ...
        DeepgramProvider(api_key = get_api_key('DEEPGRAM_API_KEY', secrets), model="nova-2"),
        DeepgramProvider(api_key = get_api_key('DEEPGRAM_API_KEY', secrets), model="nova-3"),
        
        # AssemblyAI models (v3 streaming API)
        AssemblyAIProvider(api_key = get_api_key('ASSEMBLYAI_API_KEY', secrets), model="universal-streaming"),
        
        # Speechmatics models (reduced to avoid quota issues)
        SpeechmaticsProvider(api_key = get_api_key('SPEECHMATICS_API_KEY', secrets), model="default"),
        SpeechmaticsProvider(api_key = get_api_key('SPEECHMATICS_API_KEY', secrets), model="enhanced"),
