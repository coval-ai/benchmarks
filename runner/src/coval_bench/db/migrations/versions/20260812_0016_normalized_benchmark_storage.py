...
def upgrade() -> None:
    ...
    op.execute(
        ...
        CREATE TRIGGER metric_evaluations_validate_transition BEFORE INSERT OR UPDATE ON benchmarks_v2.metric_evaluations FOR EACH ROW EXECUTE FUNCTION benchmarks_v2.validate_metric_transition();
        ...
        DECLARE target_id UUID; metric_kind TEXT; target_status TEXT; value_keys TEXT[]; primary_value DOUBLE PRECISION; component_sum DOUBLE PRECISION; primary_count INTEGER; incorrectly_designated_count INTEGER; artifact_count INTEGER;
        BEGIN
            ...
            END IF;
            SELECT metric_type, status INTO metric_kind, target_status FROM benchmarks_v2.metric_evaluations WHERE id = target_id; IF NOT FOUND THEN RETURN NULL; END IF;
            SELECT array_agg(value_key ORDER BY value_key), max(value) FILTER (WHERE value_key = 'primary'), sum(value) FILTER (WHERE value_key <> 'primary'), count(*) FILTER (WHERE is_primary), count(*) FILTER (WHERE (value_key = 'primary') <> is_primary) INTO value_keys, primary_value, component_sum, primary_count, incorrectly_designated_count FROM benchmarks_v2.metric_values WHERE metric_evaluation_id = target_id;
