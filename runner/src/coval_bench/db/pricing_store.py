...
"""``PricingStore`` — the append-only pricing log and the two doors into it.
...
describes which day. The two doors differ only in when they decline to write:

* :meth:`PricingStore.record` — the admin API. A recording identical to the
  one currently describing that effective date is a no-op (a double-submit
  writes nothing), while re-recording a value that lost to a later correction
  is a real change and appends.
* :func:`sync_packaged` — the ratesheet mirror. A packaged rate recorded at
  any point, even one an admin has since corrected, is left alone: the files
  seed the log and keep it open to review, they do not overrule it.
"""
...
from coval_bench.registries.pricing.loader import PRICING
...
# The identity a migration or the ratesheet sync writes under, in the same
# ``updated_by_user_id`` slot a Clerk subject would fill.
SYNC_IDENTITY = "pricing-sync"
...
def sync_packaged(conn: psycopg.Connection[Any], *, recorded_by: str = SYNC_IDENTITY) -> int:
    """Record every packaged ratesheet entry the log has never held; return how many.

    Idempotent: a second run over an unchanged package writes nothing. A rate
    an admin has since corrected in the log is also left alone — the files
    seed and mirror the log, they do not overrule it.
    """
    inserted = 0
    with conn.transaction():
        for key in sorted(PRICING):
            ...
            if exists is not None:
                continue
            conn.execute(_INSERT, _insert_params(new, recorded_by, None))
            inserted += 1
    return inserted
...
def pricing_sync() -> None:
    """Record the packaged ratesheets into the pricing log (idempotent)."""
    from coval_bench.config import get_settings

    with psycopg.connect(str(get_settings().database_url)) as conn:
        inserted = sync_packaged(conn)
    click.echo(f"pricing sync: {inserted} new recording(s)")
