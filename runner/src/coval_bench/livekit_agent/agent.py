...
def build_session(
    contract: Contract, voice_activity: vad.VAD, stt_keyterms: list[str]
) -> AgentSession[Any]:
    delay = endpointing_delay(contract)
    stack = contract.stack
    return AgentSession(
        stt=deepgram.STT(model=stack.stt.model, language="en", keyterm=stt_keyterms),
...
async def answer(ctx: JobContext) -> None:
    ...
    session = build_session(CONTRACT, VAD, KEYTERMS)
    await session.start(
