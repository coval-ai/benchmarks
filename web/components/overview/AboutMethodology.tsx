...
const SECTIONS: MethodologySection[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "We exclude connection setup (TCP, TLS, and protocol handshakes) the same way for every provider, so the numbers reflect the model and not the network. Distance we can't subtract: our workers run in us-east-1, so a provider serving from elsewhere carries a round-trip in every measurement, and we label the region on the model wherever they've told us. Aggregates show the full spread from p25 to p99 next to the mean, and leaderboards rank by the median.",
    ],
  },
  {
