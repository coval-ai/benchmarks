...
export function PlaygroundPageClient() {
  ...
  const visitedModesRef = useRef(new Set<"tts" | "stt">());

  useEffect(() => {
    if (visitedModesRef.current.has(mode)) return;
    visitedModesRef.current.add(mode);

    capturePostHogEvent(
      mode === "tts"
        ? POSTHOG_EVENTS.playgroundTtsVisited
        : POSTHOG_EVENTS.playgroundSttVisited,
      {
        mode,
        path: "/playground"
      }
  ...
  return (
    <PlaygroundShell header={<PlaygroundHeader />}>
