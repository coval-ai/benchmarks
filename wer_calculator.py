...
def sentence_to_numbers(sentence):
    ...
    while i < len(words):
        ...
        if i > 0 and words[i - 1].lower().rstrip(",.;:!?") in months:
            ...
            i > 1
            and words[i - 2].lower().rstrip(",.;:!?") in months
            and words[i - 1].lower() in ["the"]
        ):
            is_date_context = True
            ...
            if first_num >= 20 and first_num % 10 == 0 and first_num < 100:
                ...
                if "-" in next_word:
                    ...
                    if len(parts) == 2 and all(part in word_to_num for part in parts):
                        if (
                            word_to_num[parts[0]].endswith("0")
                            and int(word_to_num[parts[1]]) < 10
                        ):
                            second_num = int(word_to_num[parts[0]]) + int(
                                word_to_num[parts[1]]
                            )
                            if second_num < 100:
...
def normalize_text(text):
    normalized_text = squish_numbers(
        remove_punctuation(
            format_money(
                convert_ordinals(sentence_to_numbers(dehyphenate(text.lower())))
            )
        )
    )
...
def compare_transcription(original_text, transcription):
    ...
    wer, incorrect, normalized_reference, normalized_hypothesis = (
        calculate_word_error_rate(original_text, transcription)
    )
